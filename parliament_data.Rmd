...
# create a list of all unique answering bodies
answer_body <- questions_tibble |> distinct(AnsweringBody) |> pull(AnsweringBody)
answer_body

# for each answerbody, how many questions are addressed
questions_tibble |> 
  group_by(AnsweringBody) |> 
  summarise(NumQuestions = n()) |> 
  arrange(desc(NumQuestions))

# print 100 questions from Department of Health and Social Care
questions_tibble |> 
  filter(AnsweringBody == "Department of Health and Social Care") |> 
  select(QuestionText) |> 
  head(100)
```
...
3.  Create a function to retrieve target MP info from various MP API endpoints
...
| Areas of Focus                                            | /api/Members/{id}/Focus               |
...
base_url_mp <- "https://members-api.parliament.uk/api/Members"

test_url <- "https://members-api.parliament.uk/api/Members/4598/RegisteredInterests"
r <- GET(test_url)
r_json <- fromJSON(rawToChar(r$content))
r_json$value$nameDisplayAs
r_json$value$gender
r_json$value$latestParty$name
r_json$value$latestHouseMembership$house
latestHouseMembership$membershipFrom
r_json$value$name

retrieve_mp_info <- function(mp_id, base_url) {
  ...
  # then retrieve the Areas of Focus
  query_three <- sprintf("%s/%s/Focus", base_url, mp_id)
